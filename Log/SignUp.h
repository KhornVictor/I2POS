...
void get_data_from_user(string email,string password){
    ...
    do{
        ...
        cout << CYAN << "Informatiom" << RESET << endl;
        if (check) cout << BLUE << "Enter Your Information" << RESET << endl;
        else cout << RED << "Invalid age! Please try to age (0-99)!" << RESET << endl;
    ...
    ofstream file;
    file.open("Database/Admin/SignUp.csv", ios::app);
    if (!file.is_open()) cout << RED << "Cannot open the file!" << RESET << endl;
    else file << email << ',' << current_time() << endl;
    file.close();

    ofstream temporary_file;
    temporary_file.open("Database/temporary_user_info.csv", ios::out);
    if (!temporary_file.is_open()) {
        cout << RED << "Cannot open the file!" << RESET << endl;
        return;
    }
    temporary_file << email << ',' << current_time() << endl;
